...
async function getSystemPrompt() {
  if (!systemPrompt) {
    systemPrompt = await readFile(join(__dirname, '..', 'prompts', 'context.txt'), 'utf-8');
  }
  return systemPrompt;
}
 ...
 * Extracts goals, interests, constraints, and concrete values from a browser demonstration.
 */
export async function analyseContext(client, intent, events) {
  ...
  try {
    ...
    console.warn('Context Analyser returned non-JSON response:', text.substring(0, 200));
    return { raw_response: text, primary_goal: intent, sub_goals: [], constraints: [], concrete_values: {} };
  }
}
